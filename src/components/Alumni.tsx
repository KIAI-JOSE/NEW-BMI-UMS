    ...
    <div className="h-full flex flex-col animate-fade-in relative">
      ...
      <div className="flex-shrink-0 sticky top-0 z-40 bg-white/95 dark:bg-gray-900/95 backdrop-blur-md border-b border-gray-200 dark:border-gray-700 px-4 py-2 flex flex-col md:flex-row justify-between items-center gap-2 shadow-sm min-h-[60px]">
        <div className="flex items-center gap-3 pl-14 w-full md:w-auto">
           ...
           <div className="flex flex-col">
              <h2 className="text-base md:text-lg font-bold text-[#2E004F] dark:text-white tracking-tight uppercase leading-none">Global Alumni Registry</h2>
              <p className="text-[8px] md:text-[9px] font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-widest">BMI Institutional Success Network</p>
           </div>
        </div>
      ...
      <div className="sticky top-[60px] z-30 bg-[#F8F9FA]/95 dark:bg-[#0a0015]/95 backdrop-blur-md border-b border-gray-200 dark:border-gray-800 px-6 py-3 flex items-center gap-3 overflow-x-auto no-scrollbar shadow-sm">
         <div className="flex items-center gap-2 mr-4 text-gray-400">
            ...
            <span className="text-[9px] font-black uppercase tracking-widest">Graduation Cohorts</span>
         </div>
         {years.map(y => (
            <button
              key={y}
           ...
           <div className="bg-white dark:bg-gray-900 w-full max-w-xl shadow-2xl border-t-[8px] border-[#4B0082] overflow-hidden flex flex-col animate-slide-up">
              ...
              <form onSubmit={handleSave} className="p-10 space-y-8 bg-[#FAFAFA] dark:bg-gray-950 max-h-[70vh] overflow-y-auto no-scrollbar">
                 ...
                 <div className="space-y-2">
                    ...
                    <input required type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full px-5 py-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-none outline-none font-bold text-sm uppercase tracking-tight focus:border-[#4B0082]" />
                 </div>
                 {/* ... More Inputs ... */}
                 <div className="flex flex-col gap-5 pt-4">
                    <button type="submit" className="w-full py-5 bg-[#4B0082] text-white rounded-none shadow-2xl font-black uppercase tracking-[0.2em] text-xs border border-[#FFD700]/30 hover:bg-black transition-all flex items-center justify-center gap-4">
